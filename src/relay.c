...
do_usage(int exitcode)
{
	...
	printf("  -i  listen on <interface> for connections, defaults to 0.0.0.0\n");
...
main(int argc, char * const argv[])
{
	...
	server *internal_submission;
        char listeninterface[256] = "0.0.0.0";
	...
	while ((ch = getopt(argc, argv, ":hvdstf:i:p:w:b:q:H:")) != -1) {
		switch (ch) {
			...
			case 'f':
				...
				break;
			case 'i':
                                snprintf(listeninterface, sizeof(listeninterface), "%s", optarg);
				break;
			case 'p':
