...
do_start()
{
	...
	start-stop-daemon --start --quiet --pidfile $PIDFILE --user $DAEMON_USER --exec $DAEMON --test > /dev/null \
		|| return 1
	start-stop-daemon --start --quiet --pidfile $PIDFILE --user $DAEMON_USER --exec $DAEMON -- $OPTS \
		|| return 2
}
...
do_stop()
{
	...
	start-stop-daemon --stop --quiet --user $DAEMON_USER --retry=TERM/30/KILL/5 --pidfile $PIDFILE --name $NAME
	RETVAL="$?"
	[ "$RETVAL" = 2 ] && return 2
	...
	# sleep for some time.
	start-stop-daemon --stop --quiet --oknodo --user $DAEMON_USER --retry=0/30/KILL/5 --exec $DAEMON
	[ "$?" = 2 ] && return 2
